...
class SmallResidualNetwork(nn.Module):   #원래는 layer 4개. 64, 32, 16, 8이였음.
...
class NNiLQRDynamicModel(iLQRDynamicModel):
    ...
    """
    def __init__(self, network, init_state, init_action, noise_std=0.01, use_noise=False):
        """ Initialization
            network : nn.module
    ...
    def _process_data(self, dataset) -> typing.Tuple[Tensor, Tensor]:
        ...
        traj = data.state
        if self.use_noise:
            noise = torch.normal(0, self.noise_std, size=(traj.shape[0], 3), device=traj.device)
            traj[:, :3] += noise
    ...
    def pretrain(self, dataset_train: Dataset, dataset_vali: Dataset, max_epoch=100000, stopping_criterion=1e-4, lr=1e-3, model_name="NeuralDynamic.model"):
        ...
        if not os.path.exists(os.path.join(model_path, model_name)):
            logger.info("[+ +] Model file \"" + model_name +
                        "\" does NOT exist. Pre-training starts...")
            loss_fun = nn.MSELoss()
            ...
            for epoch in range(max_epoch):
                ...
                if obj_train.item() < stopping_criterion or epoch % 1000 == 0:  # Check stopping criterion
                    ## Evaluation ###
                    self._model.eval()
                    # Forward Propagation
    ...
    def retrain(self, dataset, max_epoch=1000, stopping_criterion=1e-3, lr=1e-3):
        ...
        for epoch in range(max_epoch):
            ...
            obj_train.backward()
            self.optimizer.step()
            if obj_train.item() < stopping_criterion or epoch == 0:  # Check stopping criterion
                if self.retrain_count % 50 == 0:
...
class NNiLQR(iLQRWrapper):
    ...
    def __init__(self,
        ...
        self._t = t
        self._log_once = False  #궤적확인용

    def init(self, scenario: DynamicModelBase, use_noise = False, noise_std = 0.01) -> NNiLQR:
        ...
        if barrier_obj_fun is None:
            raise ValueError("Objective function is not defined in the scenario.")
        ...
        def generate_random_trajectory():
            ...
            if not self._log_once:
                logger.info("[+ +] Generated Random Trajectory:")
                logger.info(f"Shape: {traj.shape}")  # 크기 출력
                logger.info(f"Sample Trajectory (First 5 Rows):\n{traj[:5]}")  # 샘플 데이터
                self._log_once = True  # 플래그를 True로 설정하여 이후 로그 생략
        ...
        self._nn_dynamic_model = NNiLQRDynamicModel(
            network, scenario.init_state, scenario.init_action, noise_std=noise_std, use_noise=use_noise) ####
            ...
            for i in range(int(self._iLQR_max_iter)):   
                ...
                if i ==0 or i % 25 == 24:
                    logger.info("[+ +] Total.Iter.No.:%3d   Iter.No.:%3d   Iter.Time:%.3e   Obj.Val.:%.5e" % (
                                        total_iter_no+1,       i+1,            iter_time,      obj_val,))
...
class ModelBasediLQR(iLQRWrapper):
    """This is a model-based iLQR class"""
    ...
    def init(self, scenario: DynamicModelBase) -> ModelBasediLQR:
        if not scenario.with_model() or scenario.is_action_discrete() or scenario.is_output_image():
            raise Exception("Scenario \"" + scenario.name +
                            "\" cannot learn with ModelBasediLQR")
        # Initialize the dynamic_model and objective function
        self._example_name = scenario.name
        self._dynamic_model = iLQRDynamicModel(dynamic_function=scenario.dynamic_function,
                                               x_u_var=scenario.x_u_var,
    ...
    def solve(self):
        ...
        for idx_j, j in enumerate(self._t):
            ...
            for i in range(int(self._iLQR_max_iter)):   
                ...
                if isStop:
                    new_data += [trajectory]
                    data = np.concatenate(new_data[-int(self._iLQR_max_iter/5):])[:, :, 0]
                    # Flatten the trajectory data
                    clean_trajectory = [[float(item[0]) for item in step] for step in trajectory]

                    # Save the clean trajectory data
                    logger.save_to_json(trajectory=clean_trajectory)
